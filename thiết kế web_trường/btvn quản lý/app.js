...
function showClassesForDay() {
    ...
    const selectedDay = selectedDate.toLocaleDateString('vi-VN', { weekday: 'long' });
    ...
    const normalizedDay = dayMapping[selectedDay] || selectedDay;
    console.log(`Ngày được chọn: ${normalizedDay}`);
    const classScheduleList = document.getElementById("class-list-container");
    classScheduleList.innerHTML = ''; // Xóa danh sách cũ

    const classesForDay = classes.filter(cls => cls.schedule.includes(normalizedDay));
    console.log(`Lớp học vào ngày ${selectedDay}:`, classesForDay);
    ...
    classesForDay.forEach(cls => {
        const originalIndex = classes.indexOf(cls); // Lấy chỉ số thực từ mảng gốc
        const div = document.createElement("div");
        div.className = "class-info";
        div.innerHTML = `
            <h3>${cls.className} - Phòng ${cls.room}</h3>
            <p>Thời gian: ${cls.startTime} - ${cls.endTime}</p>
            <button class="btn btn-primary" onclick="showStudentsInClass(${originalIndex})">Hiển thị học sinh</button>
            <button class="btn btn-danger" onclick="deleteClass(${originalIndex})">Xóa Lớp</button>
        `;
